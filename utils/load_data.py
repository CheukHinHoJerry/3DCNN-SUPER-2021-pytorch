...
def check_amount_h5_data(filename, datasetname):
    f = h5py.File(filename, 'r')
    data = f[datasetname][:, :, :, :]
    AMOUNT = list(data.shape)
    return AMOUNT


def enlarge(x_set, TARGET_HEIGHT, TARGET_WIDTH):
    [AMOUNT, channel, HEIGHT, WIDTH] = x_set.shape
    x_enlarge = []
    for i in range(AMOUNT):
        frame = x_set[i, 0, :, :].reshape((HEIGHT, WIDTH))
        ...
        frame_enlarge = cv2.resize(numpy.asarray(frame), dsize=(TARGET_HEIGHT, TARGET_WIDTH),
                                   interpolation=cv2.INTER_NEAREST)  # INTER_CUBIC
        x_enlarge.append(frame_enlarge)
    x_enlarge = numpy.reshape(x_enlarge, (AMOUNT, 1, TARGET_HEIGHT, TARGET_WIDTH))
    return x_enlarge


def repacking(x):
    # ----------------------------get shape from input numpy array---------------------------------
    (AMOUNT, channel , TARGET_HEIGHT, TARGET_WIDTH) = x.shape
    DEPTH = 5
    ...
    for i in range(AMOUNT):
        if (i - HALF_RANGE) < 0 or (i + HALF_RANGE) > AMOUNT + 1:
            AMOUNT = AMOUNT - 1
        else:
            if DEPTH % 2 == 0:
    ...
    package_set = numpy.reshape(package_set, (AMOUNT, channel, DEPTH, TARGET_HEIGHT, TARGET_WIDTH))
    return package_set
